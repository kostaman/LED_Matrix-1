...
The PIC32MZ NetCard supports up to 131072 pixels. The firmware will except rows up to 512 and columns up to 1280 as standard for the NetCards. However the pixel support of the PIC32MZ is five times less. It will convert any setting given which exceeds this automatically. It favors rows over columns. Meaning it will allocate the max rows supported then reduce the columns to the supported limit. Note this is still enough to support two ColorLight 5A-75B or a single ColorLight 5A-75E completely. However driving multiple receiver cards which do not use max resolution per card is also supported.
